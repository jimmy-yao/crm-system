...
public class UserServiceImpl implements UserService {
    ...
    public UserDTO createUser(UserDTO userDTO) {
        ...
        // 优化建议: 为了减少数据库交互次数，可以将以下三个查询合并为一个在Mapper中的查询，
        // 例如: int count = userMapper.countByUsernameOrEmailOrPhone(userDTO.getUsername(), userDTO.getEmail(), userDTO.getPhone());
        // 这需要修改UserMapper.java和对应的XML文件。
        // 验证用户名唯一性
        ...
        try {
            ...
            throw new RuntimeException("创建用户失败，数据库操作异常", e);
        }
    }
    ...
    public UserDTO updateUser(UserDTO userDTO) {
        ...
        if (StringUtils.hasText(userDTO.getPhone())) {
            UserDTO userWithPhone = userMapper.findByPhone(userDTO.getPhone());
            if (userWithPhone != null && !userWithPhone.getId().equals(userDTO.getId())) {
                throw new ValidationException("手机号已存在");
            }
        }
        ...
        try {
            ...
            // 优化：避免额外的数据库查询
            return userDTO.setPassword(null);
        } catch (DataAccessException e) {
            log.error("Failed to update user with ID: {}", userDTO.getId(), e);
            throw new RuntimeException("更新用户失败，数据库操作异常", e);
        }
    }
    ...
    public void deleteUser(Long id) {
        ...
        try {
            ...
            throw new RuntimeException("删除用户失败，数据库操作异常", e);
        }
    }
    ...
    public List<UserDTO> getAllUsers() {
        ...
        try {
            ...
            throw new RuntimeException("获取用户列表失败，数据库操作异常", e);
        }
    }
