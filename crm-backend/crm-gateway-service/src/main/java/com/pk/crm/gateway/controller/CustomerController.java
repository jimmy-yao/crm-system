...
public class CustomerController {
    ...
    public ResponseEntity<PageInfo<CustomerDTO>> getAllCustomers(
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "10") int size) {
        log.info("Received request to get all customers, page: {}, size: {}", page, size);
        PageInfo<CustomerDTO> customers = customerService.getAllCustomers(page, size);
        log.info("Returning {} customers", customers.getTotal());
    ...
    public ResponseEntity<PageInfo<CustomerDTO>> searchCustomers(
            @Parameter(description = "搜索关键词", required = true) @RequestParam String keyword,
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "10") int size) {
        log.info("Received request to search customers with keyword: {}, page: {}, size: {}", keyword, page, size);
        PageInfo<CustomerDTO> customers = customerService.searchCustomers(keyword, page, size);
        log.info("Found {} customers matching keyword: {}", customers.getTotal(), keyword);
        return ResponseEntity.ok(customers);
    }
}
